      ...
      function applyLoadedDataToUI() {
        ...
        document.getElementById("hide-input-title").checked =
        ...
        document.getElementById("hide-char-count").checked =
          AppState.hideCharCount;
        document.getElementById("hide-page-numbers").checked =
          AppState.hidePageNumbers;
        document.getElementById("hide-grid-guides").checked =
          AppState.hideGridGuides;
        document.getElementById("pattern-guide").checked =
          AppState.patternGuide;
        document.getElementById("pattern-empty").checked =
      ...
      function setGridColor(val, isCustom = false, shouldSave = true) {
        ...
        if (!isCustom) {
          const matchedBtn =
            ...
            document.querySelector(
              `button[onclick*="'${val}'"] .grid-color-check`,
            );
      ...
      function setGuideColor(val, isCustom = false, shouldSave = true) {
        ...
        if (!isCustom) {
          const matchedBtn =
            ...
            document.querySelector(
              `button[onclick*="'${val}'"] .guide-color-check`,
            );
          if (matchedBtn) matchedBtn.classList.remove("hidden");
      ...
      function updateHeaderAndTitle() {
        ...
        document
          .querySelectorAll(".name-placeholder-container")
          .forEach((el) => {
        ...
        document.querySelectorAll(".title-placeholder").forEach((el) => {
          el.textContent = AppState.hideInputTitle
            ? ""
            : AppState.articleTitle || "";
        });
      }
      ...
      function createGridFooter(spec, isLineNote) {
        ...
        footerDiv.className =
          "absolute bottom-[18mm] left-[20mm] right-[20mm] pt-2 flex justify-between items-center text-xs text-slate-400 font-bold shrink-0";
      ...
      function createLineNote(optRows) {
        ...
        for (let r = 0; r < optRows; r++) {
          ...
          const borderThicknessClass = isLastRow
            ? "border-b-[3px]"
            : "border-b";
      ...
      function createCharCountLabels(
        ...
        for (let r = 0; r < optRows; r++) {
          ...
          for (let k = 1; k <= currentMaxMultiplesCount; k++) {
            ...
            if (
              targetBaseValue > rowStartCell &&
              targetBaseValue <= rowEndCell
            ) {
      ...
      function buildSkeletonPage(
        ...
        const { wrapper, pageDiv, innerDiv } = createPageShell(pageClass, spec);
        ...
        if (spec.type === "source") {
          ...
          if (!AppState.hidePageNumbers) {
            ...
            pageOneFooter.className =
              "absolute bottom-[18mm] left-[20mm] right-[20mm] pt-2.5 flex justify-between items-center text-xs text-slate-400 font-bold shrink-0";
          ...
          const headerElements = createPageHeader(
            headerHTML,
            titleText,
            isLineNote,
          );
          ...
          gridWrapper.className = AppState.hideManuscriptHeader
            ? "w-full flex items-center justify-center"
            : "w-full my-0 flex items-center justify-center";
      ...
      function updatePageBadge(current, total) {
        ...
        if (
          pagesBadge &&
          (!inputField || inputField.classList.contains("hidden"))
        ) {
