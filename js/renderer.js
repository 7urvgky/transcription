...
function renderLinePageContent(wrapper, spec, linesData, optRows) {
  ...
  for (let r = 0; r < optRows; r++) {
    ...
    const lineObj = linesData[lineIdx] || {
      text: "",
      isFirstLine: false,
    ...
    if (spec.currentMode === "guide" && lineText) {
      nextState =
        lineText + "_" + lineObj.isFirstLine + "_" + lineObj.isLastLineOfPara;
    }
    ...
    if (nextState === "EMPTY") {
      ...
      textSpan.className =
        "font-serif-fixed text-slate-300 font-medium select-none absolute left-[2mm] right-[2mm] bottom-[1.8mm] leading-none pb-0 whitespace-pre line-note-text-span";
...
function renderPageContents(pageSpecs, currentLayoutSignature) {
  ...
  const wrappers = Array.from(
    container.querySelectorAll(".page-scale-wrapper"),
  );
  const pageClass =
  ...
  const cellsPerPage = isLineNote
    ? optRows
    : parseInt(AppState.gridCols) * optRows;
  ...
  if (isLineNote) {
    ...
    linesData = SourcePageEngine.splitTextForLineNote(
      AppState.sourceText,
      maxNonSpace,
    );
  } else {
...
function buildPageSpecs() {
  ...
  if (isLineNote) {
    ...
    const linesData = SourcePageEngine.splitTextForLineNote(
      AppState.sourceText,
      maxNonSpace,
    );
