...
ManuscriptEngine.calculateLineNoteAvailableHeightMm = function () {
  ...
  if (!AppState.hideManuscriptHeader) {
    ...
    measureInner.className = "print-page-inner flex flex-col min-h-0";
    ...
    if (headerElements.length > 0) {
      ...
      const lastElement = headerElements[headerElements.length - 1];

      const lastRect = lastElement.getBoundingClientRect();

      headerHeightMm =
        (lastRect.bottom - innerRect.top) * (25.4 / 96) - pageTopPaddingMm;

      headerHeightMm = Math.max(0, headerHeightMm);
    }
