...
SourcePageEngine.canFitSinglePage = function (paragraphs, fontSizePt) {
  const segments = [];
...
SourcePageEngine.paginateSourceText = function (paragraphs) {
  ...
  for (let i = 0; i < stream.length; i++) {
    ...
    const estimated = ManuscriptEngine.estimateLinesForSegments(
      candidateSegments,
      fontSizeNum,
    );
      ...
      if (currentPageSegments.length > 0) {
        ...
        if (item.type === "sentence") {
          ...
          currentPageSegments.push({
            text: item.text,
            isContinued: isContinuedOnNewPage,
            pIdx: item.pIdx,
          });
        } else {
...
SourcePageEngine.splitTextForLineNote = function (text, maxNonSpace) {
  const lines = [];
